...
function installing_onboarder() {
  ...
  if [ $yn = "Y" ]; then
    ...
    helm -n $NS install inji-certify-partner-onboarder mosip/partner-onboarder \
   ...
   ./copy_cm_func.sh secret injicertify-misp-onboarder-key inji-certify config-server

    kubectl -n config-server set env --keys=mosip-injicertify-misp-key --from secret/injicertify-misp-onboarder-key deployment/config-server --prefix=SPRING_CLOUD_CONFIG_SERVER_OVERRIDES_

    kubectl -n config-server rollout restart deploy config-server
    kubectl -n config-server get deploy -o name | xargs -n1 -t kubectl -n config-server rollout status
    kubectl rollout restart deployment -n inji-certify inji-certify

    echo inji-certify MISP License Key updated successfully.
